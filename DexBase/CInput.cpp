...
BOOL CKeyboard::KeyDown(unsigned int key)
{
	return m_keys[key] & 0x80;
}

BOOL CKeyboard::KeyUp(unsigned int key)
{
...
//-----------------------------
BOOL CInputSystem::KeyDown(unsigned int key)
{
	if(m_keyboard == NULL)
		return FALSE;
...
BOOL CInputSystem::KeyUp(unsigned int key)
{
	if(m_keyboard == NULL)
		return FALSE;
