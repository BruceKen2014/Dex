...
{
protected:
	std::list<CDexWidget*> m_WidgetList; //直属widget
	int m_totalCount; //总共的widget数量
	CDexWidget*  m_onwer; //该container所属的的widget，如果没有则为NULL
public:
	CDexWidgetContainer();
	~CDexWidgetContainer();
...
public:
	...
	void SetOnwer(CDexWidget* onwer);
	bool Empty();  //是否为空
	int  GetThisWidgetCount();  //获取直属本container下的控件数量
	int  GetTotalWidgetCount(); //获取本contianer下的所有控件数量
	void AddWidgetToFront(CDexWidget* widget);  //头部插入widget
	void AddWidgetToBack(CDexWidget* widget, bool asChild = true); //尾部插入widget  asChild:是否作为子控件
	void InsertWidgetFront(CDexWidget* wait_insert, int id);  //在列表中编号为id之前插入wait_insert
	void InsertWidgetBack(CDexWidget* wait_insert, int id);  //在列表中编号为id之后插入wait_insert
	/*
