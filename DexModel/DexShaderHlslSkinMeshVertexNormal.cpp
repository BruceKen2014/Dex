...
DexShaderHlslSkinMeshVertexNormal::DexShaderHlslSkinMeshVertexNormal()
{
	LPD3DXBUFFER	pbuff = NULL;
	HRESULT hr = D3DXCreateEffectFromFile(DexGameEngine::getEngine()->GetDevice(), "shader\\skinMeshVertexNormal.fx",
	...
	{
		getLog()->BeginLog();
		getLog()->Log(log_error, "DexShaderHlslSkinMesh D3DXCreateEffectFromFile µ÷ÓÃÊ§°Ü£¡");
		getLog()->EndLog();
	}
	else
	{
...
void DexShaderHlslSkinMeshVertexNormal::Render()
{
	...
	for (size_t i = 0; i < skinMesh->m_vecMeshs.size(); ++i)
	{
		...
		for (uint32 p = 0; p < pass; ++p)
		{
			...
			DexGameEngine::getEngine()->DrawPrimitive(DexPT_LINELIST, skinMesh->m_vecMeshs[i]->GetNormalBuffer(),
				skinMesh->m_vecMeshs[i]->GetNormalBufferCount(), skinMesh->m_vecMeshs[i]->GetNormalIndiceBuffer(),
				skinMesh->m_vecMeshs[i]->GetNormalIndiceCount() / 2, sizeof(DexSkinMesh::stMeshVertex));
			pFxEffect->EndPass();
		}
	}
	pFxEffect->End();
